...
class SelectionFunction(object):
    """Compute the fractional detectable sources of a population.

    This is a base class and does not actually perform
    the calculation.
...
class LensedBinaryBlackHoleSelectionFunctionFromMachineLearning(SelectionFunction):
    """Compute the fractional detectable lensed BBHs with `pdetclassifier`.

    To compute the selection function :math:`\beta`, which is
    the fraction of sources that would produce detectable *lensed* GW signals
    out of a population, a total of `N_inj` sources are drawn
    from a fiducial source population model, with each having `N_img`
    lensed GW siganls (or 'images') of the same source. Instead of performing
    `N_img` x `N_inj` injections and count the number of detected sources to
    get the selection function, here we use the machine-learing-based
    tool `pdetclassifier`, which can classify whether a GW signal is
    detectable or not based on its parameters (and detector sensitivity)
    without actually performing the matched-filtering search.

    This implements basically Eq (B7) of arXiv:2104.09339.

    This requires a working installation of `tensorflow`, and
    preferrably configured to work with CUDA though not required.
    Note that there is no need to install `pdetclassifier` as
    a package separately since its source code is already in
    ...
    def __init__(
        ...
        """Initialize a `LensedBinaryBlackHoleSelectionFunctionFromMachineLearning` object.

        The object can be initialized either by reading the fiducial predictions
        stored in `filename`, or by generating the fiducial predictions from scratch
        using `pdetclassifier` with the model given in `trained_model`.
        ...
        spin_src_pop_model : SourcePopulationModel
            ...
            {"spin_1x", "spin_1y", "spin_1z",
             "spin_2x", "spin_2y", "spin_2z"} is given.
        merger_rate_density_src_pop_model : MergerRateDensity
            The population model for the merger rate density.
            This object should return the probability density when
            a pandas DataFrame with the column/a dictionary with the key
            {"redshift"} is given.
        abs_magn_dist : list, optional
            The probability distribution of the absolute magnification
            for each lensed GW signal. For now, the distribution is
            assumed to be independent to each other and will be
            relaxed in future version.
        optical_depth : OpticalDepth
            The model for the strong lensing probability at a given redshift.
        filename : str, optional
            The path to a HDF5 file previously generated by the code
            storing the fiducial predictions.
        trained_model : str, optional
            The path to a HDF5 file generated by `pdetclassifier` storing
            a pre-trained neural-network classifier.
        seed : int, optional
            The (pseudo-)random seed.
        N_inj : int, optional
            The number of fiducial sources to generate for the calculation.
            The default value is set to 1e7, which is sufficient given
            that the typical value for the selection function
            is about 1e-3 - 1e-4. The default value should give
            a decent number of effective samples.
        N_z : int, optional
            The number of redshift samples to be used when performing the
            Monte-Carlo integration. The default value here is 100.
        N_img : int, optional
            The number of lensed GW siganls produced per source.
    ...
    def make_fiducial_predictions(self):
        """Generate some fiducial lensed GW sources and determine if they are detectable.

        Here we use the same reweighting technique as in Eq (B3) of arXiv:2104.09339
        to estimate the inner integral in Eq (B7) of the same paper for each redshift.
        The fiducial distriubtion (:math:`q(\theta_i)`) for the component masses
        is a distribution that is uniform in component masses.
        The fiduical distribution for the component spins is
        uniform in spin magnitude and isotropic in spin orientation.
        
        For each fiducial source, `N_img` lensed GW signals are generated,
        where the apparent luminosity distances are drawn from a fiduical
        distribution that is uniform in co-moving volume. Note that
        the detectability depends only on 'detector-frame' quantities.

        The fiducial predictions made will be saved to a HDF5 file
    ...
    def evaluate(self):
        """Evaluate the fractional detectable BBHs using the fiducial predictions.

        This function is using the RHS of Eq (B7) in arXiv:2104.09339.
        The selection function :math:`\beta` is calculated as follows.
        For each redshift sample (`N_z` of them in total), we first
        compute the inner integral :math:`\epsilon(z)` 
        using Monte-Carlo integration. In the expression for :math:`\epsilon(z)`,
        :math:`p_{\rm det}` is the prediction from `pdetclassifier`
        which must be 1 (detectable) if a fiducial source was saved to the HDF5 file.
        The weighting factor :math:`p_{\rm pop}/q` is then summed over and divided
        by `N_inj` to give an estimate of the inner integral.

        We then use Monte-Carlo integration again and average over
        :math:`\epsilon(z)` for different redshifts drawn from the
        source redshift distribution p(z) and give an estimate
        of the overall 'outer' integral :math:`\beta`, which is the selection function.

        Returns
        -------
        beta : float
            The selection function/detectable fraction for lensed BBHs.
        """
        self.load_from_file()
...
class BinaryBlackHoleSelectionFunctionFromMachineLearning(SelectionFunction):
    """Compute the fractional detectable BBHs with `pdetclassifier`.

    To compute the selection function :math:`\alpha`, which is
    the fraction of sources that would produce detectable GW signals
    out of a population, a total of `N_inj` sources are drawn
    from a fiducial source population model. Instead of performing
    `N_inj` injections and count the number of detected signals to
    get the selection function, here we use the machine-learing-based
    tool `pdetclassifier`, which can classify whether a GW signal
    is detectable or not based on its parameters (and detector sensitivity)
    without actually performing the matched-filtering search.

    This implements basically Eq (B3) of arXiv:2104.09339.
    ...
    def __init__(
        ...
        """Initialize a `BinaryBlackHoleSelectionFunctionFromMachineLearning` object.
    ...
    def make_fiducial_predictions(self):
        """Generate some fiducial GW sources and determine if they are detectable.

        The fiducial distriubtion (:math:`q(\theta_i)` in Eq (B3) of arXiv:2104.09339)
        for the component masses is a distribution that is
        uniform in component masses. The fiduical distribution for
        the component spins is uniform in spin magnitude and
        isotropic in spin orientation. The fiducial distribution
        for the redshift is uniform in co-moving volume.
    ...
    def evaluate(self):
        ...
        This function is using the RHS of Eq (B3) in arXiv:2104.09339.
        In the expression, :math:`p_{\rm det}` is the prediction from
        `pdetclassifier` which must be 1 (detectable) if a fiducial
        source was saved to the HDF5 file. The weighting factor
        :math:`p_{\rm pop}/q` is then summed over and divided
        by `N_inj` to give an estimate of the selection function.

        Returns
        -------
        alpha : float
            The selection function/detectable fraction.
        """
        self.load_from_file()
